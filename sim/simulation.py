...
from module_sampling import sample_module_positions, build_connectivity_graph, build_networkx_graph, detect_communities
...
def run_full_simulation(params, config, apply_laplacian, domain, Fx_init, Fy_init, W_init, eta_init, output_dir, 
                        initial_modules=None, initial_weights=None):
    ...
    NetworkX_graphs = []  # Store NetworkX graph objects for visualization/analysis
    Communities_series = []  # Store community detection results
    ...
    if initial_modules is not None:
        module_coords = initial_modules
        print(f"Using {len(module_coords)} pre-initialized module positions from NeuroML")
    else:
    ...
    # Build initial connectivity graph and communities
    if initial_weights is not None:
        weights = initial_weights
        print(f"Using pre-initialized connectivity weights from NeuroML")
        
        # Build NetworkX graph
        G = build_networkx_graph(module_coords, weights)
        communities, community_map = detect_communities(G)
        
        Graphs.append(weights.clone().cpu().numpy())
        NetworkX_graphs.append(G)
        Communities_series.append(communities)
    ...
    visualize_simulation(
        ...
        NetworkX_graphs=NetworkX_graphs, Communities=Communities_series, bio_prior=bio_prior
    )
